...
public class Controlador implements ActionListener {
    ...
    public Controlador(VentanaPrincipal ventanaPrincipal, PersonaNegocio personaNegocio) {
        ...
        this.pnlModificarPersona.getListPersonas()
                .addListSelectionListener(a -> getSelectedValueJList_ModificarPersona(a));
        this.pnlModificarPersona.getBtnModificar()
                .addActionListener(a -> EventoClickBoton_PanelModificar_btnModificar(a));
    ...
    public void EventoClickBoton_PanelAgregar_btnAceptar(ActionEvent A) {
        ...
        if (!Nombre.isEmpty() && !Apellido.isEmpty() && !Dni.isEmpty()) {
            if (personaNegocio.insert(p)) {
                ...
                resultado = "Hubo un error, el dni ya existe en los registros";
        } else
            resultado = "Es necesario completar todos los campos";
    ...
    public void EventoClickBoton_PanelEliminar_btnEliminar(ActionEvent a) {
        ...
        if (p == null) {
            ...
            if (personaNegocio.delete(p))
                resultado = "Persona eliminada con exito";
            else
...
class soloNumeros extends KeyAdapter {
    ...
    public void keyTyped(KeyEvent e) {
        ...
        if (!(Character.isDigit(key)))
            e.consume();
        if (texto.getText().trim().length() == 8)
            e.consume();
    }
